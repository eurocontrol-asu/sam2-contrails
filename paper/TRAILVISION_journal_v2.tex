...
Instance segmentation has progressed from encoder--decoder networks~\cite{ronneberger2015u} to region-proposal methods~\cite{he2017maskrcnn} and transformer architectures such as Mask2Former~\cite{cheng2021maskformer, cheng2021mask2former_video}, which has been applied directly to video contrail segmentation~\cite{jarry2025gvccs} with strong per-frame accuracy but no built-in attribution. In video object segmentation, memory-augmented models such as XMem~\cite{cheng2022xmem}, Cutie~\cite{cheng2024cutie}, and DeAOT~\cite{yang2022deaot} propagate one or a few annotated frames through a video; none is designed to incorporate external domain knowledge that specifies object locations on every frame.

The Segment Anything Model (SAM)~\cite{kirillov2023sam} introduced promptable segmentation, and SAM2~\cite{ravi2024sam2} extends it to video with a temporal memory bank: a vision backbone (Hiera~\cite{ryali2023hiera}) extracts image features, a prompt encoder embeds the user's spatial input, and a mask decoder combines both with memory to produce segmentations. In standard use, a human annotates one or two \emph{conditioning frames} and SAM2 propagates through the rest. Recent adaptations cover camouflaged object segmentation~\cite{zhou2024sam2cos}, semantic segmentation~\cite{hesham2025sam2vss}, and more~\cite{zhang2025sam2survey}; to our knowledge ours is the first driven by per-frame, non-visual, physics-derived prompts. This reversal of the usual sparse-annotation ratio creates the problems that the modifications in Section~\ref{sec:architecture} address.

\subsection{Contrail Attribution}

Attribution algorithms match detected contrails to flights using geometric criteria~\cite{chevallier2023linear, riggi2023ai, geraedts2024scalable, dalmau2025attribution}: trajectories are projected into image coordinates, advected by wind, compared with detected shapes, and matched by solving an assignment problem, on geostationary satellite data~\cite{chevallier2023linear, geraedts2024scalable}, with probabilistic treatments of uncertainty~\cite{riggi2023ai}, and from ground cameras in our previous framework~\cite{dalmau2025attribution}. A common assumption across these works---including our own---is that detection and tracking are perfect: the attribution algorithm receives clean masks and unbroken tracks. In practice they are not, and errors in either stage propagate silently. The present paper removes this assumption by evaluating the complete pipeline from raw images to flight-level attribution.
...
\paragraph{Augmentation.} All spatial transforms (random flips, affine rotations of $\pm 25^{\circ}$) are applied with identical parameters to every frame in a sample---and to the prompt and ground truth masks---to preserve temporal and physical consistency. Color jitter is applied in two stages: a consistent pass across frames simulates different sky conditions, and a smaller per-frame pass mimics frame-to-frame illumination shifts from auto-exposure or passing clouds.
...
The gains in Table~\ref{tab:ablation} are close to additive: the negative signal and the shorter window each contribute at every point of the grid, with only mild synergy between spatial precision and the age gradient. The paired bootstrap of Section~\ref{sec:uncertainty} confirms that these within-grid effects are statistically robust rather than resampling noise.
...
\paragraph{Architectural modifications evaluated jointly.} The five SAM2 modifications are applied as a package; leave-one-out ablations are absent, so the claim that each transfers to other physics-prompted settings rests on mechanistic reasoning rather than direct evidence.
