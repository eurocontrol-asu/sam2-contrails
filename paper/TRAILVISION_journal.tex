...
\begin{abstract}
A contrail cannot exist without a flight. We exploit this constraint by inverting the usual detect-then-attribute pipeline: instead of finding contrails in an image and asking which flight made them, we start from each flight and ask whether its contrail is visible. Flight positions from air traffic surveillance data and wind fields from meteorological reanalysis feed a contrail lifecycle model that predicts where each flight's contrail should appear in the camera image---not at the aircraft's current position, but where atmospheric transport has carried the exhaust plume. These predicted locations become spatial hints (prompts) that guide a video object segmentation model to either confirm the contrail by outlining its boundaries in the image or reject the prediction when no contrail is visible. Because each prompt corresponds to exactly one flight, there is no post-hoc matching step: the output mask carries the flight's identity by construction, eliminating the appearance-based re-identification errors that compound in sequential pipelines, and tracking across frames is implicit---no separate identity-matching stage is needed. We compare prompts that carry increasing amounts of physical information---from simple presence indicators to age-weighted encodings that signal contrail freshness, optionally layered with suppression at locations claimed by other flights. On the GVCCS ground-camera dataset, the richest prompt design improves mAP$_{25:75}$ by 25\% relative to a binary presence baseline, achieves 96.2\% attribution precision (the fraction of detections matched to the correct flight), and maintains detection, on average, across three-quarters of each contrail's visible lifetime; video-level bootstrap confidence intervals confirm that the gains over the binary baseline are statistically robust.
\end{abstract}
...
\caption{System overview. ADS-B positions and ERA5 wind fields feed the CoCiP contrail lifecycle model, whose predicted trajectories are projected into image coordinates to produce per-flight spatial masks. Three prompt encodings are evaluated: binary, age-weighted, and age-weighted with negative signal layering, at two age windows (5 and 10~min). An adapted Segment Anything Model 2 (SAM2) model (with re-enabled memory attention, frame-distance temporal encoding, softened memory sigmoid, and dice-dominant loss rebalancing) receives these dense per-frame prompts and outputs per-flight segmentation masks.}
...
We manually categorized the 50 lowest-performing flights (by temporal IoU) from the best variant to understand where the system fails.

Wind estimation errors cause roughly one-third of failures. ERA5 wind fields miss mesoscale variability, and errors exceeding $\sim$5~m/s displace prompts by tens of pixels from the actual contrail. When displacement exceeds the model's spatial tolerance, the prompt provides misleading guidance and the model either rejects a real contrail (thinking the prompt is wrong) or segments the wrong region.
