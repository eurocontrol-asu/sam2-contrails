    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                # Collect per-object score logits (available when pred_obj_scores=True)
                if "object_score_logits" in current_out:
                    object_score_logits_per_obj[obj_idx] = current_out[
                        "object_score_logits"
                    ]
            ...
            # Stack per-object score logits into [num_objects, 1] tensor (or None)
            if any(s is not None for s in object_score_logits_per_obj):
                all_object_score_logits = torch.cat(
                    object_score_logits_per_obj, dim=0
                )
            else:
                all_object_score_logits = None
            yield frame_idx, obj_ids, video_res_masks, all_object_score_logits
