...
class Trainer:
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                # Determine if this is an optimizer step boundary
                is_accum_step = (
                    (data_iter + 1) % self.gradient_accumulation_steps == 0
                ...
                # Log schedulers (per-iteration, independent of accumulation)
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
    ...
    def _run_step(
        ...
        # zero_grad is now controlled by the training loop for gradient accumulation
        with torch.cuda.amp.autocast(
        ...
        # Divide loss by accumulation steps so accumulated gradients average correctly
        if self.gradient_accumulation_steps > 1:
            loss = loss / self.gradient_accumulation_steps
        self.scaler.scale(loss).backward()
        # Log the un-scaled loss (multiply back) for correct loss reporting
        loss_mts[loss_key].update(
            loss.item() * self.gradient_accumulation_steps, batch_size
        )
