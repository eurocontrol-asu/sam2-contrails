...
def hflip(datapoint, index):
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is not None:
            data = obj.segment.permute(2, 0, 1)
            data = F.hflip(data)  # Expects [..., H, W]
            obj.segment = data.permute(1, 2, 0)
...
class RandomAffine:
    ...
    def transform_datapoint(self, datapoint: VideoDatapoint):
        ...
        for img_idx, img in enumerate(datapoint.frames):
            # Permute [H,W,2] → [2,H,W] so F.affine treats last two dims as spatial
            # Same pattern as hflip/vflip transforms
            this_masks = [
                obj.segment.permute(2, 0, 1) if obj.segment is not None else None
            ...
            for i in range(len(img.objects)):
                if this_masks[i] is None:
                    ...
                    # Check visibility using channel 0 (GT mask) only, not the prompt channel
                    if img_idx == 0 and transformed_mask[0].max() == 0:
                        # We are dealing with a video and the object is not visible in the first frame
                        # Return the datapoint without transformation
                        return None
                    # Permute back [2,H,W] → [H,W,2]
                    transformed_masks.append(transformed_mask.permute(1, 2, 0))
