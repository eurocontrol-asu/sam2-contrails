...
class SAM2DensePromptTrain(SAM2Train):
    """
    SAM2 adaptation for training with dense prompt masks.

    Supports two modes:
    - All frames have prompts (original dense conditioning)
    - Mixed: some frames have prompts (conditioning), others don't (memory propagation)

    Uses input.has_prompt [T, O] to determine which frames are conditioning frames.
    Frames without prompts are processed as non-conditioning frames, teaching the
    model to propagate from memory and use pred_obj_scores to stop predicting.
    """
    ...
    def prepare_prompt_inputs(
        ...
        """
        Prepares mask inputs based on 'input.prompts' and 'input.has_prompt'.

        Frames where has_prompt is True get mask_input (conditioning frames).
        Frames where has_prompt is False get no mask_input (memory propagation).
        """
        # 1. Load Ground Truth masks (for loss calculation later)
        gt_masks_per_frame = {
            stage_id: masks.unsqueeze(1) for stage_id, masks in enumerate(input.masks)
        }
        ...
        if not hasattr(input, "prompts"):
            raise AttributeError(
                "BatchedVideoDatapoint needs 'prompts' attribute for SAM2DensePromptTrain mode."
            )

        # 2. Split frames into conditioning (have prompts) and non-conditioning
        # has_prompt: [T, O] — a frame is conditioning if ANY object has a prompt
        cond_frames = []
        non_cond_frames = []
        for t in range(num_frames):
            if input.has_prompt[t].any():
                cond_frames.append(t)
            else:
                non_cond_frames.append(t)

        # 3. Only provide mask_inputs for conditioning frames
        backbone_out["mask_inputs_per_frame"] = {
            t: input.prompts[t].unsqueeze(1)
            for t in cond_frames
        }
        backbone_out["point_inputs_per_frame"] = {}

        backbone_out["init_cond_frames"] = cond_frames
        backbone_out["frames_not_in_init_cond"] = non_cond_frames

        # 4. Disable correction click simulation
    ...
    def forward_tracking(
        ...
        """
        Forward video tracking with mixed conditioning/propagation frames.

        Processes all frames sequentially (0 to T-1). For each frame:
        - Conditioning frames (has_prompt=True): uses prompt as mask_input
        - Non-conditioning frames (has_prompt=False): propagates from memory only
        """
        img_feats_already_computed = backbone_out["backbone_fpn"] is not None
        feat_sizes = None
        ...
        num_frames = backbone_out["num_frames"]
        cond_frame_set = set(backbone_out["init_cond_frames"])
        ...
        # Process ALL frames sequentially to maintain temporal order for memory
        for stage_id in range(num_frames):
            ...
            is_cond = stage_id in cond_frame_set
        ...
        # Format output as list for loss computation (maintain frame order)
        all_frame_outputs = {}
        all_frame_outputs.update(output_dict["cond_frame_outputs"])
        all_frame_outputs.update(output_dict["non_cond_frame_outputs"])
        all_frame_outputs = [all_frame_outputs[t] for t in range(num_frames)]

        # Remove unused obj_ptr key for compatibility with DDP + activation checkpointing
        all_frame_outputs = [
            {k: v for k, v in d.items() if k != "obj_ptr"} for d in all_frame_outputs
        ]
    ...
    def _iter_correct_pt_sampling(self, *args, **kwargs):
        """Disabled: no iterative point-based correction in dense prompt mode."""
        pass
