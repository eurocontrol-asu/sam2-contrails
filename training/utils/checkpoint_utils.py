...
def load_state_dict_into_model(
    ...
    # Drop checkpoint keys whose shapes don't match the model.
    # This turns size mismatches into missing keys (which ignore_missing_keys can handle)
    # instead of hard RuntimeErrors from load_state_dict.
    model_state = model.state_dict()
    for key in list(state_dict.keys()):
        if key in model_state and state_dict[key].shape != model_state[key].shape:
            logging.warning(
                f"Dropping '{key}' from checkpoint: shape {state_dict[key].shape} "
                f"doesn't match model shape {model_state[key].shape}"
            )
            del state_dict[key]
